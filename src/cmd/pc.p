...
class ParasolCommand extends process.Command {
	public ParasolCommand() {
		...
		includeOption = stringOption('i', "include",
					"Include the following source directory in the compile, as if it were in a used package.");
	...
	ref<process.Option<string>> includeOption;
...
void parseCommandLine(string[] args) {
	...
	if (parasolCommand.includeOption.set()) {
		includes := parasolCommand.includeOption.value.split(':');
		missing := false
		for (i in includes) {
			include := includes[i];
			if (include.length() == 0)
				continue;
			if (!storage.isDirectory(include)) {
				missing = true;
				printf("Included %s is not a directory.\n", include);
				continue;
			}
			parasolCommand.includes.append(include);
		}
		if (missing)
			parasolCommand.help();
	}
...
int runCommand() {
	...
	if (parasolCommand.pxiOption.set()) {
		...
		if (idx >= 0) {
			...
			if (!context.Version.isValid(pxiVersion)) {
				printf("Cannot create pxi with a version of '%s'\n", pxiVersion);
				return 1;
			}
		} else {
	...
	compileContext.includes = parasolCommand.includes;
