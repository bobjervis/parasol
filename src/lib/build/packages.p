...
//		printf("setVersion %s -> %s\n", toString(), version);
	...
	ref<Product>[], boolean listDependencies() {
		return _includedProducts, true;
	}
...
//			printf("    %d Looking for %s -> %s\n", thread.currentThread().id(), _name, _usedPackageNames[i]);
			...
			if (p == null) {
				...
				return _includedProducts, false;
			}
...
class MakeProduct extends Product {
	...
	public boolean build() {
		...
		if (coordinator().officialBuild()) {
			storage.deleteFile(target)
			storage.deleteFile(product)
		}
 ...
 */
	...
	ref<Product>[], boolean listDependencies() {
		_includedProducts.append(_package)
		return _includedProducts, true;
	}
}
