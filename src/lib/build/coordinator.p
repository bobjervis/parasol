		...
		if (_commandLineProducts.length() > 0) {
			...
			do {
				...
				for (i in selected) {
					...
					checked[i] = true;
					changed = true;
					included := _products[i].listDependencies();
					for (j in included) {
...
*/
	...
	public int run() {
		...
		ProductSorter sorter(_products);
		
		_products = sorter.sort()
...
class ProductSorter {
	int[] _level;		// The level of each product. A product with no dependencies has level 0, a product with dependencies
						// has a level thaat i s one more than the highest level amognst it's dependencies.
	ref<Product>[] _products;
	int[string] _indexMap;
	ref<Product>[][] _stacks;
			
	ProductSorter(ref<Product>[] products) {
		_products = products;
		_level.resize(products.length())
		_stacks.resize(products.length())
		for (i in _products) {
			p := _products[i]
			_level[i] = -1
			_indexMap[p.toString()] = i
		}
	}
	...
	ref<Product>[] sort() {
		for (i in _products)
			if (_level[i] < 0)
				_level[i] = collect(i)
		for (i in _products)
			_stacks[_level[i]].append(_products[i])

		ref<Product>[] sorted			
		for (i in _stacks)
			for (j in _stacks[i]) {
//				printf("level %d: %s\n", i, _stacks[i][j].toString())
				sorted.append(_stacks[i][j])
			}
	...
	int collect(int i) {
		p := _products[i]
		dependencies := p.listDependencies()
		maxLevel := -1;
		for (j in dependencies) {
			dep := dependencies[j]
			if (dep == null)
				throw exception.IllegalArgumentException("Product " + i + " in dependencies for " + p.toString() + " is null")
			name := dep.toString()
			if (!_indexMap.contains(name))
				throw exception.IllegalArgumentException("Product " + dep.toString() + " in dependencies but not in full product list")
			idx := _indexMap[name]
			level := collect(idx)
			if (level > maxLevel)
				maxLevel = level
