...
//	}
	 ...
	 *
	 * @return The signal number of the signal that caused any thread in the process yo stop. 
...
//				logger.info("-->> pid %d tid %d STARTED", _process.id(), _tid);
	...
	public void printStackTrace() {
		...
		string name;
		long offset;
		(seg, sym, name, offset) = mm.findSymbol(regs.rip, 0);
		if (name != null) {
			if (offset == 0)
				offStr := ""
			else
				offStr.printf("+%x", offset)
			logger.info("*-> %s%s\n", name, offStr);
		} else
			logger.info(" -> %p rbp %p (no symbol cause %d)\n", regs.rip, regs.rbp, offset);
